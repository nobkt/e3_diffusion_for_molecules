...
def get_dataset_info(dataset_name, remove_h):
    if dataset_name == 'qm9':
        ...
        # For ASE databases, return a generic configuration
        # The actual config will be generated by ASEDatabaseReader
        return get_ase_dataset_info(dataset_name, remove_h)
    else:
        raise Exception("Wrong dataset %s" % dataset_name)


def get_ase_dataset_info(dataset_name, remove_h):
    """
    Generic configuration for ASE database datasets.
    This will be updated by the actual ASE database reader.
    """
    return {
        'name': dataset_name,
        'atom_encoder': {'H': 0, 'C': 1, 'N': 2, 'O': 3, 'F': 4},  # Default, will be updated
        'atom_decoder': ['H', 'C', 'N', 'O', 'F'],  # Default, will be updated
        'max_n_nodes': 50,  # Default, will be updated
        'n_nodes': {},  # Will be populated by ASE reader
        'atom_types': {},  # Will be populated by ASE reader
        'colors_dic': ['#FFFFFF99', 'C7', 'C0', 'C3', 'C1'],  # Default colors
        'radius_dic': [0.46, 0.77, 0.77, 0.77, 0.77],  # Default radii
        'with_h': not remove_h
    }
